...
class GradingSession:
    def __init__(self, grading_handler, latency=0.05, spiffy=False, topo_map="test/tmp3/topo3.map",
                 nodes_map="test/tmp3/nodes3.map"):
        self.peer_list = dict()
    ...
    def run_grader(self):
        ...
        if not self.spiffy:
            ...
            simulator_process = subprocess.Popen(cmd.split(" "), stdin=subprocess.PIPE, stdout=outfile, stderr=outfile,
...
def drop_handler(recv_queue, send_queue):
    ...
    while True:
        ...
        if pkt.pkt_type == 3 and cnt == 50 and not dropped:
